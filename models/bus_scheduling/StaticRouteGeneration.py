...
def route_combine(r_g_result, max_route, hub_id,Q_tohub,Q_fromhub): 
    ...
    print(candidate_routes_fromhub['route'])
    print(candidate_routes_tohub['route'])
...
#Attention Start: you need to redefine Q_fromhub, Q_tohub as the data from the row_idx by chaging row_idx
        ...
        for i in range(len(Q_fromhub)):
            ...
            Q_fromhub[i] =max(abs(Q_from.iloc[(locationIDList.index(objectID))*2+index_date_month+hour_idx][2]*\
                bus_ratio_file[str(keep_index[hub_id])][str(keep_index[i])])+abs(Q_from.iloc[(locationIDList.index(objectID))*2+\
        ...
        for i in range(len(Q_tohub)):
            ...
            #if objectID in columnList:
            #Q_fromhub[i] = abs(Q_from.iloc[row_idx][columnList.index(objectID)]*)+abs(Q_from.iloc[row_idx+1][columnList.index(objectID)]) 
            Q_tohub[i] =max(abs(Q_to.iloc[(locationIDtoList.index(objectID))*2+index_date_month+hour_idx][2]*\
                bus_ratio_file[str(keep_index[i])][str(keep_index[hub_id])])+abs(Q_to.iloc[(locationIDtoList.index(objectID))*2+\
	...
	#2. implement 
		...
		#3. transforms the zone index in result from (ROW_Index-1, e.g., 114(JFK)) to (OBJECTID, e.g., 120(LGA)).
                  ...
                  for i in range(20):
                      ...
                      #print(route_from)
                      #print(route_to)
                      if (len(route_from)>0)&(len(route_to)>0):
