...
class METSRClient(threading.Thread):
    ...
    def handle_step_message(self, decoded_msg):
        with self.lock: # for thread safety
            ...
            if not self.ready: 
                self.send_step_message(0) # This will initialize the simulator data structures if it is first ran, otherwise, it will be ignored
                self.ready = True
            if tick > self.current_tick: # tick smaller or equal to the current_tick is ignored
    ...
    def send_query_message(self, msg): # asynchronized, other tasks can be done while waiting for the answer
        ...
        while(self.latest_ans_message is None or self.latest_ans_message['TYPE'] != ans_type):
            time.sleep(0.001)
            if time.time() - self.prev_time > self.retry_threshold:
    ...
    def send_control_message(self, msg): # synchronized, wait until receive the answer
        ...
        while(self.latest_ctrl_message is None or self.latest_ctrl_message['TYPE'] != msg['TYPE']):
            time.sleep(0.001)
